...
class Lucene extends Object {
    ...
    public function __construct($config = null, &$backend = null) {
        ...
        if ( $backend === null ) {
            if ( $this->config['backend'] === 'SolrLuceneBackend' && ini_get('allow_url_fopen') ) {
                $backend = new SolrLuceneBackend($this);
            } elseif ( $this->config['backend'] === 'JavaLuceneBackend' && ini_get('allow_url_fopen') ) {
     ...
     * @return DataObjectSet
     */
    public function findWithSort($query_string, $sortField, $sortOrder=false, $start=null, $rows=null, $extra=array()) {
        $hits = false;
        if ( $this->getConfig('search_cache') ) {
            $hits = SearchCache::getCached($query);
        }
        if ( $hits === false ) {
            $hits = $this->backend->findWithSort($query_string, $sortField, $sortOrder, $start, $rows, $extra);
        }
        if ( $this->getConfig('search_cache') ) {
            SearchCache::cache($query, $hits);
        }
    ...
    public static function getAllIndexableObjects($className='DataObject') {
        ...
        foreach( $extendedClasses as $className ) {
            ...
            $objects->where(isset($config['index_filter']) ? $config['index_filter'] : '');
