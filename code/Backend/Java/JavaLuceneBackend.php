...
class JavaLuceneBackend extends LuceneBackend {
    ...
    public function __construct($frontend, $config = null) {
        ...
        @include_once( dirname(__FILE__).'/Java.inc' );

        // Check if our IndexWriter needs attention - if things died on a previous
        // request, the IndexWriter creation will fail.  If it does, restarting the
        // server should help.
        try {
            ...
            try {
                // If we failed, try to restart the servlet
                $this->stopStandalone();
                $this->startStandalone();
            } catch( JavaException $e ) {
                echo $e;
    ...
    public static function checkPrerequisites($debug=false) {
        ...
        if ( $debug ) {
            if ( $allow_url_fopen ) {
                echo '<p>PHP ini-value <kbd>allow_url_fopen</kbd> is enabled.  ' .
                    'This is the correct setting.</p>';
            } else {
                echo '<p>PHP ini-value <kbd>allow_url_fopen</kbd> is disabled.  ' .
                    'To use the Java backend, please enable this PHP option in your ' .
                    'php.ini or .htaccess file.</p>';
            }
        }
    ...
    /**
     * Stops the Java standalone server.
