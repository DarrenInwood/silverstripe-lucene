...
class LuceneSearchable extends DataExtension {
    ...
    private function setLuceneFieldConfig($config) {
        ...
        $config = array_merge(array_flip($this->getExtraSearchFields()), $config);
	      ...
	      foreach( $config as $fieldName => $data ) {
            ...
            // Default to unstored indexing for standard SilverStripe fields
            if ( !isset($data['stored']) && in_array($fieldName, self::$unstored_fields) ) {
                $data['stored'] = 'false';
            }
            // Default to unindexed indexing for standard SilverStripe fields
            if ( !isset($data['indexed']) && in_array($fieldName, self::$unindexed_fields) ) {
                $data['indexed'] = 'false';
            }
            // Default to keyword indexing for standard SilverStripe fields
            if ( !isset($data['type']) && in_array($fieldName, self::$keyword_fields) ) {
                $data['type'] = 'string';
            }
            // Implementations of LuceneBackend will have to decide what to do
            // with fields that have unspecified types.
            // Eg. date types in SolrLuceneBackend need a filter to format dates correctly.

            // Add defaults into anything that isn't already set
            $data = array_merge($tmp, $data);

            // Backwards compatibility with older versions of this module
            if ( $data['type'] !== null ) {
                switch( $data['type'] ) {
                    case 'keyword':
                        $data['type'] = 'string';
            ...
            $this->fieldConfig[$fieldName] = $data;
	      }
    }
